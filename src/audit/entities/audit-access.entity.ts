...
export class AuditAccess {
    ...
    @Column({ length: 128, nullable: true }) username: string | null;
    
    @Column({ length: 64 }) action: string;
    @Column({ length: 256 }) resource: string;
    @Column({ length: 8, nullable:true }) statusCode: number | null;
    @Column({ name: 'status_codde', nullable: true }) statusCode: number | null;
